...
function getTxtMod($ttype=0){ //0:absolute, 1:delta
	$timebase = $this->timebase;
	$tracks = $this->tracks;
	$tc = count($tracks);
	$type = ($tc>1)?1:0;
	$str =  "MFile $type $tc $timebase<br>";
	for ($i=0;$i<$tc;$i++) $str .= $this->getTrackTxtMod($i,$ttype);
	return $str;
}
...
function getTrackTxtMod($tn,$ttype=0){ //0:absolute, 1:delta
	$track = $this->tracks[$tn];
	$str = "MTrk<br>";
	if ($ttype==1) {//time as delta
...
function playFile($file) {
	...
	/*<script>MIDIjs.play(<?php echo "'".$file."'"; ?>);</script>*/
	?>
